...
namespace Topshelf.Model
{
    ...
    public class AppDomainShelfReference :
		ShelfReference
	{
		static readonly ILog _log = LogManager.GetLogger("Topshelf.Model.AppDomainShelfReference");
		...
		public AppDomainShelfReference(string serviceName, IsolationLevel isolationLevel, UntypedChannel controllerChannel)
		{
			_serviceName = serviceName;
			_isolationLevel = isolationLevel;
			_controllerChannel = controllerChannel;

			_domainSettings = ConfigureAppDomainSettings(_serviceName, _isolationLevel);
		...
		static AppDomainSetup ConfigureAppDomainSettings(string serviceName, IsolationLevel isolationLevel)
		{
		...
		{
			_log.DebugFormat("[{0}] Creating shelf proxy: {1} ({2})", _serviceName, address, pipeName);
